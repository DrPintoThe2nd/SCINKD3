...
#add psuedocount integer
eps <- 1e-6

# Output prefix
out_prefix <- sub("\\.bed\\.gz$|\\.gz$|\\.bed$", "", basename("SCINKD3"))
png_file <- paste0(out_prefix, ".males.png")
pdf_file <- paste0(out_prefix, ".males.pdf")
...
# --- M negate F ---
mf <- (male_norm + eps) / (female_norm + eps)
...
plot_ratio <- function() {
  plot(mf,
       ...
       ylab = "Corrected M/F Read Depth",
       main = paste("Corrected M/F SCINKD3 Depth (",
