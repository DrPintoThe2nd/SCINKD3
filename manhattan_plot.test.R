...
#calculate 95% CI value
val <- mean(log2(df$value))
sigval <- 2*sd(log2(df$value)) + mean(log2(df$value))

plot_manhattan <- function() {
  plot(df$cum_pos, log2(df$value),
       col = df$col,
       pch = 16,
       xaxt = "n",
       xlab = "Chromosome",
       ylab = "log2(Read Depth)",
  ...
  abline(h = sigval, col = "red", lty = 2, lwd = 2)
  abline(h = val,  lty = 3)
