...
out_prefix <- sub("\\.bed\\.gz$|\\.gz$|\\.bed$", "", basename(infile))
png_file <- file.path(out_dir, paste0(out_prefix, ".manhattan.png"))
pdf_file <- file.path(out_dir, paste0(out_prefix, ".manhattan.pdf"))
...
# Restrict to first X number of chromosomes
...
epsilon <- 1e-6
median_val <- median(df$value + epsilon, na.rm = TRUE)
df$value <- (df$value + epsilon) / median_val

# Replace non-finite values (Inf, -Inf, NaN, NA) with 0
df$value[!is.finite(df$value)] <- 0

plot_manhattan <- function() {
  plot(df$cum_pos, df$value/median(df$value, na.rm = TRUE),
       col = df$col,
       pch = 16,
#      cex = 0.6,
       xaxt = "n",
